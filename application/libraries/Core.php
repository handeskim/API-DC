...
class Core extends MY_Controller {
	...
	public function _card_fee($type){
			try{
			$obj_core = $this->mongo_db->where(array('card_type'=>(int)$type,))->get('card');
			if(!empty($obj_core)){
				return $obj_core[0];
			}
		}catch (Exception $e) { $this->objects; }
		return $this->objects;
	}
	...
	public function _Service_Alego_ByCard_Sendding($param,$Func){
		...
		$RefNumber = $this->_alego_transaction_create($param);
    ...
    $agentId = $alego_conf['agentid'];
		...
		$url = $alego_conf['url'];
		$data_post = $this->_alego_postUrl($url,$input);
		if(!empty($data_post['EncData'])){
			$this->obj = json_decode($this->_alego_decrypt($data_post['EncData']),true);
		}
		return $this->obj;
	}
